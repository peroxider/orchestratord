"""ReasonixSession — wraps ``reasonix`` into the AgentSession SPI.
...
is intentionally conservative — §8.1 marks the reasonix event stream as
...
class ReasonixSession:
    """Adapt ``reasonix`` subprocess output into the AgentSession SPI."""
        ...
        try:
                ...
                self._events.append(
                    EventEnvelope(
                        ...
                        payload={
                            "code": "reasonix_exit",
                            "message": (
                                f"reasonix exited with code {proc.returncode}; "
            ...
            self._events.append(
                EventEnvelope(
                    ...
                    timestamp=self._now(),
                    kind=EventKind.ERROR,
                    payload={"code": "reasonix_spawn_error", "message": str(exc)},
                )
            )
