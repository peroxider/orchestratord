"""CopilotSession — wraps ``copilot`` into the AgentSession SPI.
...
is intentionally conservative — §8.1 marks the copilot event stream as
"needs experimentation" so we do not yet claim ``streaming_deltas``.
"""
...
class CopilotSession:
    """Adapt ``copilot`` subprocess output into the AgentSession SPI."""
        ...
        try:
                ...
                self._events.append(
                    EventEnvelope(
                        ...
                        payload={
                            "code": "copilot_exit",
                            "message": (
                                f"copilot exited with code {proc.returncode}; "
            ...
            self._events.append(
                EventEnvelope(
                    ...
                    timestamp=self._now(),
                    kind=EventKind.ERROR,
                    payload={"code": "copilot_spawn_error", "message": str(exc)},
                )
            )
