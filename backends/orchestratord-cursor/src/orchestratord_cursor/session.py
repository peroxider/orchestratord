"""CursorSession — wraps ``cursor-agent`` into the AgentSession SPI.
...
is intentionally conservative — §8.1 marks the cursor event stream as
...
class CursorSession:
    """Adapt ``cursor-agent`` subprocess output into the AgentSession SPI."""
        ...
        try:
            ...
            self._events.append(
                EventEnvelope(
                    ...
                    timestamp=self._now(),
                    kind=EventKind.ERROR,
                    payload={"code": "cursor_spawn_error", "message": str(exc)},
                )
            )
