"""ZeroclawSession — wraps ``zeroclaw`` into the AgentSession SPI.
...
is intentionally conservative — §8.1 marks the zeroclaw event stream as
...
class ZeroclawSession:
    """Adapt ``zeroclaw`` subprocess output into the AgentSession SPI."""
        ...
        try:
                ...
                self._events.append(
                    EventEnvelope(
                        ...
                        payload={
                            ...
                            "message": f"zeroclaw exceeded {timeout:.1f}s",
                        },
                    )
                )
                self._events.append(
                ...
                self._events.append(
                    EventEnvelope(
                        ...
                        payload={
                            "code": "zeroclaw_exit",
                            "message": (
                                f"zeroclaw exited with code {proc.returncode}; "
            ...
            self._events.append(
                EventEnvelope(
                    ...
                    timestamp=self._now(),
                    kind=EventKind.ERROR,
                    payload={"code": "zeroclaw_spawn_error", "message": str(exc)},
                )
            )
    ...
    async def probe_resume(self) -> ResumeStatus:
        """zeroclaw has no cross-process resume probe."""
