"""KiroSession — wraps ``kiro`` into the AgentSession SPI.

Spawn-per-turn Cli model: ``send`` runs the subprocess to completion and
buffers the entire stdout as a single TEXT event. Event stream shape
is intentionally conservative — §8.1 marks the kiro event stream as
"not yet exercised" so we do not yet claim ``streaming_deltas``.
"""
...
class KiroSession:
    """Adapt ``kiro`` subprocess output into the AgentSession SPI."""

    def __init__(self, spec: SessionSpec) -> None:
        self._spec = spec
        self.session_id = spec.resume_session_id or f"kiro-{id(self)}"
        ...
        try:
            ...
            try:
                ...
                self._events.append(
                    EventEnvelope(
                        ...
                        payload={
                            "code": "kiro_timeout",
                            "message": f"kiro exceeded {timeout:.1f}s",
                        },
                    )
                )
                self._events.append(
                ...
                self._events.append(
                    EventEnvelope(
                        ...
                        payload={
                            "code": "kiro_exit",
                            "message": (
                                f"kiro exited with code {proc.returncode}; "
            ...
            self._events.append(
                EventEnvelope(
                    ...
                    timestamp=self._now(),
                    kind=EventKind.ERROR,
                    payload={"code": "kiro_spawn_error", "message": str(exc)},
                )
            )
    ...
    async def probe_resume(self) -> ResumeStatus:
        """kiro has no cross-process resume probe."""
