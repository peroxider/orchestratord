"""KimiSession — wraps ``kimi`` into the AgentSession SPI.
...
is intentionally conservative — kimi does not yet declare
``streaming_deltas`` in the descriptor (FEATURE_GAP §8.1 has no entry
for kimi's wire format).
"""
...
class KimiSession:
    """Adapt ``kimi`` subprocess output into the AgentSession SPI."""
        ...
        try:
                ...
                self._events.append(
                    EventEnvelope(
                        ...
                        payload={
                            "code": "kimi_exit",
                            "message": (
                                f"kimi exited with code {proc.returncode}; "
            ...
            self._events.append(
                EventEnvelope(
                    ...
                    timestamp=self._now(),
                    kind=EventKind.ERROR,
                    payload={"code": "kimi_spawn_error", "message": str(exc)},
                )
            )
