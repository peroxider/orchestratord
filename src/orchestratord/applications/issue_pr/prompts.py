"""issue→PR 业务的 prompt 模板与渲染器（DESIGN §4.5 / §6 / P2）。

P4 起，本模块是业务 prompt 的正式归宿（原临时模块
``business_prompts.py`` 按 DESIGN §7 P4 并入于此）。本模块在 import
时把业务模板注册进 kernel 的
:class:`~orchestratord.kernel.prompt_core.PromptRouter`（幂等），
组合根（orchestration_subsystem / applications 注册表）import 本模块
即完成装配。
...
from orchestratord.premise_check import build_premise_block, check_issue_premise
from orchestratord.prompt_builder import PromptBuilder, _to_jinja_value
...
#: 注册进 PromptRouter 后即成为业务可独立演进的接缝；P4 起归宿
#: 本模块）。
