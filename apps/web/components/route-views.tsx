...
import {
  AgentDetail, AgentsList, AuditList, AutopilotDetail, AutopilotsList, ChatPage, InboxList,
...
import { resolveResource } from '@/lib/registered-applications'
...
const collections = {
  ...
  projects: C({ eyebrow: 'SYSTEM / SHARED CONTEXT', title: 'Projects', description: 'Repositories, documents, and execution context shared across workflows.' }, { eyebrow: '系统 / 共享上下文', title: '项目', description: '集中管理各业务流共享的仓库、文档与执行上下文。' }, { eyebrow: 'システム / 共有コンテキスト', title: 'プロジェクト', description: '業務フロー間で共有するリポジトリ、文書、実行コンテキストです。' }),
...
export function InboxRouteView() { const i = useInstanceContext(); const registry = useApplicationRegistry(); const locale = useLocale(); return <Collection copy={collections.inbox}><InboxList client={apiClient} workspaceId={i.workspace_id} resolveResource={ref => resolveResource(registry, ref, locale)} /></Collection> }
...
export function AutopilotsRouteView() { const i = useInstanceContext(); const registry = useApplicationRegistry(); const platformTargets: TargetDescriptor[] = [{ id: 'platform.autopilot.squad', applicationId: null, kind: 'squad', label: { en: 'Squad', 'zh-CN': 'Agent 小组', ja: 'Squad' }, order: 10 }]; return <Collection copy={collections.autopilots}><AutopilotsList client={apiClient} workspaceId={i.workspace_id} targets={[...platformTargets, ...registry.autopilotTargets()]} /></Collection> }
...
export function ActivityRouteView() { const i = useInstanceContext(); const registry = useApplicationRegistry(); const locale = useLocale(); return <Collection copy={collections.activity}><AuditList client={apiClient} workspaceId={i.workspace_id} resolveResource={ref => resolveResource(registry, ref, locale)} /></Collection> }
export function SessionDetailRouteView({ id }: { id: string }) { const registry = useApplicationRegistry(); return <SessionDetail client={apiClient} sessionId={id} resolveSource={(session, locale) => session.origin.kind === 'resource' ? resolveResource(registry, session.origin.source, locale).label : ({ en: 'Direct conversation', 'zh-CN': '直接对话', ja: '直接会話' })[locale]} /> }
