...
const copy = {
  en: { eyebrow: 'ORCHESTRATION / EXECUTIONS', title: 'Execution sessions', note: 'Live work, waiting decisions, and durable execution evidence in one ledger.', filters: ['All', 'Running', 'Waiting', 'Failed', 'Completed'], groups: ['Active', 'Waiting for input', 'Queued', 'Failed', 'Completed'], count: 'sessions', columns: ['Session', 'Source', 'Mode', 'Status', 'Started'], error: 'Sessions could not be loaded.', errorNote: 'Your data is safe. Check the local API, then retry.', retry: 'Retry', empty: 'No sessions yet', emptyNote: 'Start an issue or conversation to create an auditable execution record.', issue: 'Issue', direct: 'Direct chat' },
  'zh-CN': { eyebrow: '编排 / 执行', title: '执行会话', note: '在同一本账本中查看实时工作、待决事项和持久化执行证据。', filters: ['全部', '运行中', '等待中', '失败', '已完成'], groups: ['活跃', '等待输入', '排队中', '失败', '已完成'], count: '个会话', columns: ['会话', '来源', '模式', '状态', '开始时间'], error: '无法加载会话。', errorNote: '数据未受影响。请检查本地 API 后重试。', retry: '重试', empty: '暂无会话', emptyNote: '启动任务或对话后，这里会生成可审计的执行记录。', issue: '任务', direct: '直接对话' },
  ja: { eyebrow: 'オーケストレーション / 実行', title: '実行セッション', note: '進行中の作業、判断待ち、永続的な実行証跡を一つの台帳で確認します。', filters: ['すべて', '実行中', '待機中', '失敗', '完了'], groups: ['実行中', '入力待ち', 'キュー', '失敗', '完了'], count: 'セッション', columns: ['セッション', 'ソース', 'モード', '状態', '開始日時'], error: 'セッションを読み込めませんでした。', errorNote: 'データは変更されていません。ローカル API を確認して再試行してください。', retry: '再試行', empty: 'セッションはまだありません', emptyNote: 'Issue または会話を開始すると、監査可能な実行記録が作成されます。', issue: 'Issue', direct: '直接チャット' },
} as const
